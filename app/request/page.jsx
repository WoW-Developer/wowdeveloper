...
const postdata = async (ymail, yname, yphone, yrequest) => {
  ...
  const { data, error } = await supabase
    .from("Requests")
    .insert([{ email: ymail, name: yname, message: yrequest, phone: yphone }]);
  ...
  return (
    <>
      <div
        id="fc"
        style={{ display: "none" }}
        className="fixed flex-col top-0 left-0 h-screen flex items-center justify-center w-screen z-[21000] bg-white "
      >
        <p className="text-primary max-w-screen-sm">
          {" "}
          Thank you! Have a Nice Time Ahead!
        </p>
        <Link
          href="/"
          className="bg-primary w-fit mx-auto hover:bg-black focus:bg-black focus:ring-0 focus:outline-none self-center p-2 mt-2 rounded text-white"
        >
          Home
        </Link>
      </div>
      <div className="h-full min-h-fit pt-14 w-full flex items-center justify-center">
        <div className="bg-white w-full max-w-[600px] relative text-black mx-4">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              submitdata(mail, name, phone, request);
            }}
            style={{ display: "none" }}
            id="root"
            className="w-full p-3 col-1 mx-auto text-center"
          >
            <h1 className="mx-auto text-center p-3 pb-0 mb-2 font-semi-bold text-xl">
              Request
            </h1>

            <div className="relative border rounded p-2 border-primary">
              <input
                tabIndex={0}
                autoComplete="off"
                type="email"
                id="mail"
                placeholder="e.g. someone@wxd.com"
                className=" rounded peer bg-transparent focus:outline-none focus:ring-0 placeholder:text-sm placeholder:text-transparent w-full"
                onChange={(e) => {
                  e.preventDefault();
              ...
              <label
                htmlFor="mail"
                className="absolute pointer-events-none cursor-text transition bg-white px-2  
after:content-['*'] after:ml-0.5 after:text-red
scale-75 -top-2.5 -left-0.5 text-dark_grey peer-placeholder-shown:text-light_grey peer-placeholder-shown:scale-100 peer-placeholder-shown:bg-transparent peer-placeholder-shown:top-1.5 peer-placeholder-shown:left-0 duration-300 ease-in-out"
              >
                E-Mail
              </label>
            </div>
            <h1
              id="mailerror"
              style={{ display: "none" }}
              className="mr-auto w-fit text-red text-xs"
            >
              Enter Valid Email
            </h1>
            ...
            <div className="relative border rounded border mt-2 p-2 border-primary">
              <input
                ...
                id="name"
                placeholder="e.g. someone@wxd.com"
                className=" required rounded peer bg-transparent focus:outline-none focus:ring-0 placeholder:text-sm placeholder:text-transparent w-full"
                onChange={(e) => {
                  e.preventDefault();
              ...
              <label
                htmlFor="name"
                className="absolute pointer-events-none cursor-text transition bg-white px-2 after:content-['*'] after:ml-0.5 after:text-red scale-75 -top-2.5 -left-0.5 text-dark_grey peer-placeholder-shown:text-light_grey peer-placeholder-shown:scale-100 peer-placeholder-shown:bg-transparent peer-placeholder-shown:top-1.5 peer-placeholder-shown:left-0 duration-300 ease-in-out"
              >
                Name
              </label>
            </div>
            ...
            <h1
              id="nameerror"
              style={{ display: "none" }}
              className="mr-auto text-red w-fit text-xs"
            >
              Enter Name
            </h1>
            ...
            <div className="relative border rounded border mt-2 p-2 border-primary">
              <input
                ...
                className=" rounded peer required bg-transparent focus:outline-none focus:ring-0 placeholder:text-sm placeholder:text-transparent w-full"
                onChange={(e) => {
                  e.preventDefault();
...
*/}
            ...
            <div className="relative border rounded border mt-2 p-2 border-primary">
              <textarea
                ...
                className=" rounded required peer bg-transparent focus:outline-none focus:ring-0 placeholder:text-sm placeholder:text-transparent w-full"
                onChange={(e) => {
                  e.preventDefault();
          ...
          <div
            id="spin"
            className="flex flex-col  mx-auto  justify-center items-center h-fit w-fit"
          >
            <svg
